    ...
    internal class T3BilheteLayout : BaseLayout<T3Bilhete>
    {
        public T3BilheteLayout()
        {
            this.WithMember(x => x.DataVencimento           , c=>c.WithLength(8 ).WithTypeConverter<DateConverter>())
                ...
                .WithMember(x => x.Categoria                , c=>c.WithLength(3 ))
                .WithMember(x => x.DescricaoCategoria       , c=>c.WithLength(50))
                ...
                .WithMember(x => x.SinalValorLigacao        , c=>c.WithLength(1 ).WithTypeConverter<SinalConverter>())
                .WithMember(x => x.AliquotaIcms             , c=>c.WithLength(5 ))
                .WithMember(x => x.ValorLigacaoComImposto   , c=>c.WithLength(13))
